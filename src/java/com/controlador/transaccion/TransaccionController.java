package com.controlador.transaccion;

import com.entity.transaccion.Transaccion;
import com.controlador.transaccion.util.JsfUtil;
import com.controlador.transaccion.util.JsfUtil.PersistAction;
...
public class TransaccionController implements Serializable {
    ...
    @EJB
    private com.controlador.transaccion.TransaccionFacade ejbFacade;
    ...
    public void create() {
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/Bundle_transaccion").getString("TransaccionCreated"));
    ...
    public void update() {
        persist(PersistAction.UPDATE, ResourceBundle.getBundle("/Bundle_transaccion").getString("TransaccionUpdated"));
    }

    public void destroy() {
        persist(PersistAction.DELETE, ResourceBundle.getBundle("/Bundle_transaccion").getString("TransaccionDeleted"));
        if (!JsfUtil.isValidationFailed()) {
        ...
        if (selected != null) {
            ...
            try {
                ...
                if (msg.length() > 0) {
                    ...
                    JsfUtil.addErrorMessage(ex, ResourceBundle.getBundle("/Bundle_transaccion").getString("PersistenceErrorOccured"));
                }
            } catch (Exception ex) {
